...
def train_dann(model, params, src_data_loader, tgt_data_loader, tgt_data_loader_eval, device, logger, attack=None,fgsm_model=None):
    """Train dann."""
    ####################
    # 1. setup network #
    ####################
    ...
    for epoch in range(params.num_epochs):
        ...
        for step, ((images_src, class_src), (images_tgt, class_tgt)) in data_zip:
            ...
            # Call FGSM Attack
            if attack=="fgsm":
                images_tgt = fgsm_attack(fgsm_model,alpha,criterion,images_tgt,class_tgt,0.25,device)
            """
            ...
            logger.add_scalar('src_discriminator_loss_', src_loss_domain.item(), global_step)
            logger.add_scalar('tgt_discriminator_loss', tgt_loss_domain.item(), global_step)
        ...
        if ((epoch + 1) % params.eval_step == 0):
            ...
            logger.add_scalar('src_classification_acc', src_acc, global_step)
            logger.add_scalar('src_discriminator_acc', src_acc_domain, global_step)
            logger.add_scalar('tgt_test_loss', tgt_test_loss, global_step)
            logger.add_scalar('tgt_classification_acc', tgt_acc, global_step)
            logger.add_scalar('tgt_discriminator_acc', tgt_acc_domain, global_step)
